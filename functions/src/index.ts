...
import type { ImprovementAction, ActionComment, SentEmailInfo } from "../../src/lib/types";
import { sendDueDateReminderEmail } from "../../src/services/notification-service";
...
type DueDateSettings = z.infer<typeof DueDateSettingsSchema>;
...
// Helper flows
async function getDueDateSettingsFlow(): Promise<DueDateSettings> {
    const docRef = db.collection('app_settings').doc('due_date_reminders');
...
// Main logic
async function checkDueDatesFlow(input: z.infer<typeof CheckDueDatesInputSchema>): Promise<z.infer<typeof CheckDueDatesOutputSchema>> {
    let settings;
    try {
