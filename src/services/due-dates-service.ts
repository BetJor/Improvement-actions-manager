...
import { db as clientDb } from '@/lib/firebase'; // Client SDK instance

// This file can run in two environments:
// 1. Next.js server-side ('use server'): It will use the client SDK (clientDb).
// 2. Cloud Function environment: It will use the Admin SDK (admin.firestore()).
// The `getDb` function handles this distinction.
const getDb = (): Firestore => {
    return admin.apps.length ? admin.firestore() : clientDb;
}
